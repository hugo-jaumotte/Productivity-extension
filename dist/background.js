...
function createBlockedSiteRegex(domains) {
...
async function updateBlockingRules(blockSites) {
    ...
    const rules = blockedSitesWithRegex.map((site, index) => ({
        id: index + 1,
        priority: 1,
        action: {
...
chrome.webNavigation.onBeforeNavigate.addListener(async (details) => {
    ...
    const sites = await getBlockedSites();
    const blockedSitesWithRegex = createBlockedSiteRegex(sites);
    const matchedSite = blockedSitesWithRegex.find(site => new RegExp(site.regex).test(details.url));
    if (matchedSite) {
        const blockedPageUrl = chrome.runtime.getURL('pages/blocked.html');
...
chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (changes.blockSites) {
        updateBlockingRules(changes.blockSites.newValue);
    }
